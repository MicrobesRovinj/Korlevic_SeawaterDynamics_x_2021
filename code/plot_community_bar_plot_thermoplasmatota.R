#################################################################################################################
# plot_community_bar_plot_thermoplasmatota.R
# 
# A script to plot sequence abundances of groups from the phylum Thermoplasmatota of each sample.
...
# Produces: results/figures/thermoplasmatota_bar_plot.jpg
#
#################################################################################################################
...
select <- filter(community,
                 taxlevel==6 &
                   str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Thermoplasmatota$"))$rankID, "\\."))) %>%
...
plot <- filter(community,
  ...
  bind_rows(summarise_all(., list(~ifelse(is.numeric(.), 100-sum(.), paste("Other_Thermoplasmatota"))))) %>%
...
names <- parse(text=case_when(str_detect(plot$taxon, "uncultured") ~ paste0("plain('Uncultured')~italic('", str_remove(plot$taxon, "uncultured_"), "')"),
                              ...
                              plot$taxon=="Marine_Group_II_ge" ~ "plain('Marine Group II')",
                              plot$taxon=="Other_Thermoplasmatota" ~ "plain('Other')~italic('Thermoplasmatota')",
...
# Saving
ggsave("results/figures/thermoplasmatota_bar_plot.jpg", p, width=297, height=210, units="mm")