#################################################################################################################
# plot_community_bar_plot_actinobacteriota.R
# 
# A script to plot sequence abundances of groups from the phylum Actinobacteriota of each sample.
...
# Produces: results/figures/actinobacteriota_bar_plot.jpg
#
#################################################################################################################
...
select <- filter(community,
               taxlevel==6 &
               str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Actinobacteriota$"))$rankID, "\\."))) %>%
...
plot <- filter(community,
  ...
  bind_rows(summarise_all(., list(~ifelse(is.numeric(.), 100-sum(.), paste("Other_Actinobacteriota"))))) %>%
...
names <- parse(text=case_when(str_detect(plot$taxon, "uncultured") ~ paste0("plain('Uncultured')~italic('", str_remove(plot$taxon, "uncultured_"), "')"),
                              ...
                              plot$taxon=="Candidatus_Actinomarina" ~ "plain('\"')*italic('Candidatus')~plain('Actinomarina\"')",
                              plot$taxon=="Other_Actinobacteriota" ~ "plain('Other')~italic('Actinobacteriota')",
...
p <- ggplot(plot) +
  ...
  geom_text(data=whole,
            aes(x=date, y=88, label=paste(format(round(abundance, 1), nsmall=1), "%")),
...
# Saving
ggsave("results/figures/actinobacteriota_bar_plot.jpg", p, width=297, height=210, units="mm")