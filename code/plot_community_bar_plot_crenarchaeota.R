#################################################################################################################
# plot_community_bar_plot_crenarchaeota.R
# 
# A script to plot sequence abundances of groups from the phylum Crenarchaeota of each sample.
...
# Produces: results/figures/crenarchaeota_bar_plot.jpg
#
#################################################################################################################
...
select <- filter(community,
                 taxlevel==6 &
                   str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Crenarchaeota$"))$rankID, "\\."))) %>%
...
plot <- filter(community,
               taxlevel==6 &
                 str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Crenarchaeota$"))$rankID, "\\."))) %>%
  mutate_at(5:ncol(.), list(~. / sum(.) * 100)) %>%
  mutate_at(5:ncol(.), list(~ replace_na(., 0))) %>%
  ungroup() %>%
  filter(rankID %in% select$rankID) %>%
  bind_rows(summarise_all(., list(~ifelse(is.numeric(.), 100-sum(.), paste("Other_Crenarchaeota"))))) %>%
...
names <- parse(text=case_when(str_detect(plot$taxon, "uncultured") ~ paste0("plain('Uncultured')~italic('", str_remove(plot$taxon, "uncultured_"), "')"),
                              ...
                              plot$taxon=="Candidatus_Nitrosopumilus" ~ "plain('\"')*italic('Candidatus')~plain('Nitrosopumilus\"')",
                              plot$taxon=="Other_Crenarchaeota" ~ "plain('Other')~italic('Crenarchaeota')",
...
# Saving
ggsave("results/figures/crenarchaeota_bar_plot.jpg", p, width=297, height=210, units="mm")