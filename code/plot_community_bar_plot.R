...
plot <- filter(community, taxlevel==2 |
  ...
  mutate_at(5:ncol(.), list(~case_when(taxon=="Proteobacteria" ~ . - sum(.[taxlevel==3 &
    str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Proteobacteria$"))$rankID, "\\."))]), TRUE ~ .))) %>%
...
# Saving
ggsave("results/figures/community_bar_plot.jpg", p, width=297, height=210, units="mm")