...
select <- filter(community,
               taxlevel==6 &
               str_detect(rankID, paste0(filter(community, str_detect(taxon, "^Bacteroidota$"))$rankID, "\\."))) %>%
...
# Removing the last digit from rankID so for the next step
...
# Saving
ggsave("results/figures/bacteroidota_bar_plot.jpg", p, width=297, height=210, units="mm")